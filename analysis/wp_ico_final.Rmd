...
this description can be quite verbose. Thus, we will provide both a [quick
summary](#quick_summary) and a [deep dive](#deep_dive) on our approach.
...
page count effect. We select the model that
minimizes [cross-validated root mean squared error (CV-RMSE)](https://en.wikipedia.org/wiki/Root-mean-square_deviation) 
under a 5-fold simulation. Since we are predicting log-amount raised, we
...
This is severe underfitting of the valuation process, and we think it is
...
log-amount raised with respect to page count could potentially violate our
current statistical tests. As a followup, we should consider a
...
current model.
2. CV-RMSE suggests that our current model is off on average
...
3. If we want to consider a more feature-rich regression model, we would
...
4. On a similar note, we should consider other model families besides
linear models for performing this regression once we have more features. As we
increase the size of our feature set, we could automate interaction
...
white paper, it is possible that page count is a noisy measure of true
white paper length. As a robustness check, we may be interested in modeling the
effect of word count on \$ amount raised in ICO rather than the effect
of page count.
6. For a causal narrative, we are interested in spending more time
